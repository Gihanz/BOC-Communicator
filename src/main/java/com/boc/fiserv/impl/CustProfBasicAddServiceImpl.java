...
*/
public class CustProfBasicAddServiceImpl extends BaseServiceImpl implements CustProfBasicAddService
{
	...
	public CustomerProfileAddRs execute(Object cifDetails) throws JAXBException,Exception
	{
		...
		String inputXML = createCustProfBasicAddRqJaxbObject(cifDetails);
		log.info("inputXML is "+inputXML);
		if(null!=inputXML)
			output = invokeCustProfBasicAddRq(inputXML);
	...
	public CustomerProfileAddRs invokeCustProfBasicAddRq(String inputXML)
	{
		ResourceBundle rsbundle = getResourceBundle();
		...
		try {
			...
			CustProfBasicAddRs custProfBasicAddRsObj =  (CustProfBasicAddRs) generateOutputObject(removedIFX,CustProfBasicAddRs.class);
			if(null!=custProfBasicAddRsObj)
			{
				statusCode = custProfBasicAddRsObj.getStatus().getStatusCode();
				if(null!=statusCode && statusCode==0)
				{
					cifNo = custProfBasicAddRsObj.getCustId().getCustPermId();
					custResponse.setCifNo(cifNo);
				}
				else
				{
					custResponse.setErrorCode(custProfBasicAddRsObj.getStatus().getError().get(0).getErrNum());
					custResponse.setErrorDescription(custProfBasicAddRsObj.getStatus().getError().get(0).getErrDesc());
				}
			}
	...
	public String createCustProfBasicAddRqJaxbObject(Object cifDetails) throws JAXBException,Exception
	{
		ResourceBundle rsbundle = getResourceBundle();
		...
		try
		{
			...
			//if("".equalsIgnoreCase(cifId))
			{
				...
				else if(null!=customerType && customerType.equalsIgnoreCase("N"))
				{
					...
					if(jCIFObject.has(rsbundle.getString("CASCIFACCOUNTTYPE")))
						accountType = (String)jCIFObject.getString(rsbundle.getString("CASCIFACCOUNTTYPE"));
				...
				custProfBasicAdd.setCustProfBasic(custProfBasicType);
				inputXML = generateInputXML(custProfBasicAdd);
